...
import { getAuth, authHeaders, refreshToken } from './oauth.js';
...
export async function analyzeAccount(context, onDelta) {
  ...
  if (!auth) {
    throw new AiError('Claude is not connected yet.', { code: 'no-key' });
  }
  ...
  const send = () => fetch(API_URL, { method: 'POST', headers: authHeaders(auth), body });

  let res;
  try {
    res = await send();
    // An OAuth token can expire mid-session; refresh once and retry
    if (res.status === 401 && auth.mode === 'oauth' && await refreshToken()) {
      auth = await getAuth();
      res = await send();
    }
  } catch (e) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403) {
      throw new AiError('Claude rejected the credential. Connect again.', { code: 'auth' });
    }
    if (res.status === 429) {
