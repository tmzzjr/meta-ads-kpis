...
export async function getResultsTotal(accountId, { since, until }) {
  ...
  const prefs = await getPreferences();
  let total = 0;
  const kinds = {};   // how many results came from each kind of conversion
  ...
  for (const row of rows) {
    ...
    let kind = configured ? resultKind(prefs.bookingActionType) : '';
    if (!count) {
      const detected = detectResultAction(actions);
      count = detected.count;
      kind = resultKind(detected.type);
    }
    if (!count) continue;
...
// Normalizes a Meta action_type into the kind of result it represents, so the
// UI can name the card after what is actually being counted
export function resultKind(actionType) {
  ...
  if (type.includes('complete_registration')) return 'registration';
  if (type.includes('submit_application')) return 'application';
  if (type.includes('start_trial')) return 'trial';
  if (type.includes('subscribe')) return 'subscription';
  if (type.includes('contact')) return 'contact';
  if (type.includes('add_to_cart')) return 'add_to_cart';
  if (type.includes('custom')) return 'conversion';
  return 'result';
}
...
// Like getAds, but expands the creative so a human (or the analyst agent) can
// read the actual copy — headline, primary text and CTA. Meta stores this in
// two places depending on the ad's build path: flat fields on the creative for
// older ads, and nested under object_story_spec.link_data for most. We pull
// both and normalize in extractCreative.
export async function getAdsWithCreatives(adsetId, range) {
  const nodes = await paginate(`/${adsetId}/ads`, {
    fields:
      'id,name,status,effective_status,' +
      'creative{title,body,object_type,call_to_action_type,' +
      'object_story_spec{link_data{message,name,description,call_to_action{type}}}}',
    limit: 200
  });
  const withCreative = nodes.map(n => ({ ...n, creative: extractCreative(n.creative) }));
  return withInsights(withCreative, `/${adsetId}`, 'ad', range);
}

// Collapses Meta's two creative shapes into { title, body, cta }
function extractCreative(creative) {
  if (!creative) return null;
  const link = creative.object_story_spec?.link_data || {};
  const title = creative.title || link.name || '';
  const body = creative.body || link.message || link.description || '';
  const cta = creative.call_to_action_type || link.call_to_action?.type || '';
  if (!title && !body && !cta) return null;
  return { title, body, cta, object_type: creative.object_type || '' };
}
