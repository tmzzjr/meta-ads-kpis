...
async function postToken(payload) {
  ...
  // Preferred path: the service worker. Resolves to undefined when an older
  // worker without this handler is still running, so treat that as a miss.
  try {
    res = await chrome.runtime.sendMessage({ type: 'oauth-token', url: TOKEN_URL, payload });
  } catch {
    res = null;
  }

  // Fallback: call it straight from here. Works whenever host_permissions
  // grant the CORS bypass, which covers a stale or sleeping worker.
  if (!res) {
    try {
      ...
      throw new Error(
        'Could not reach the token endpoint. Open chrome://extensions, press reload (↻) on this extension, then try again.'
      );
    }
  }

  if (!res.ok) {
    // Authorization codes are single use and short lived, so most failures
    // here mean "go get a fresh code" rather than "retry this one"
    // Anthropic answers 429 to every code exchange on the public Claude Code
    // client, from any IP and regardless of waiting, so this is a block rather
    // than a temporary limit. Say so and send the user to the key path.
    if (res.status === 429) {
      const err = new Error('Anthropic is refusing this authorization (it returns 429 to every attempt, from any network). Use an API key instead.');
      err.code = 'blocked';
      throw err;
    }
    if (res.status === 400 || res.status === 401) {
      throw new Error('That code was rejected. Codes work once and expire quickly, so press Authorize again and paste the new one.');
    }
