...
export const STORAGE_KEYS = {
  ...
  // Claude credential for the AI analyst: { mode: 'oauth'|'key', token, ... }
  // Local only, never synced.
  ANTHROPIC_AUTH: 'anthropic_auth',
  // PKCE verifier, held only between opening the authorize page and pasting the code
  OAUTH_VERIFIER: 'oauth_verifier',
  // Legacy plain API key, migrated into ANTHROPIC_AUTH on first read
  ANTHROPIC_KEY: 'anthropic_key',
  // Set while the user is off authorizing, so the popup reopens straight into
  // the paste step instead of losing the flow
...
export const DEFAULT_PREFERENCES = {
  ...
  kpiOrder: [
    ...
    'ctr', 'cpc', 'cpm', 'reach',
    'frequency', 'bookings', 'cost_per_booking', 'roas'
  ],
  ...
  // Language the AI analyst writes its read in
  insightsLanguage: 'en',
  // Local traffic-manager server. When set, "Analyze campaigns" routes the deep
  // analysis (down to ads + creatives) through it instead of the in-browser read.
  analystServer: 'http://localhost:8787',
